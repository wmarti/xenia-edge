...
// Safety of reading a page is established by the caller, not by asking the heap.
//
...
// [address, end_address] via TranslateVirtual to decide where the function ends.
// So that exact range has provably been read without faulting, and copying it
// is no more dangerous than the scan that just happened. Nothing outside that
// range is touched -- no read-ahead margin -- because a page past the function
// end may never have been read by anyone.
...
void JitCorpusWriter::RecordFunction(Memory* memory, uint32_t address,
  ...
  // has no trustworthy extent: record it so the replay can report it, but do not
  // guess which pages to copy. end < start really happens -- the scanner emits
  // start-4 when the first word it fetches is zero.
...
std::unique_ptr<JitCorpus> JitCorpus::Read(const std::filesystem::path& path) {
  ...
  for (;;) {
    ...
    if (tag == kTagPage) {
      ...
      corpus->functions_.push_back({record[0], record[1], record[2], record[3]});
    } else {
