...
void Win32A64CodeCache::InitializeUnwindEntry(
  ...
  // Only the HostToGuest thunk saves the full callee-saved set, so only it
  // needs the full unwind description. The emitter marks it explicitly: frame
  // size cannot distinguish it (a guest function with 144 bytes of locals has
  // the same 224-byte frame) and neither can prolog size (the guest prolog runs
  // PushStackpoint, so it is far longer than the 4 instructions this used to
  // assume).
  // Other thunks (GuestToHost, ResolveFunction) have different layouts but are
  // only called from within JIT'd code; stack-alloc-only unwind info is
  // sufficient for them since the unwinder will walk up to the HostToGuest
  // frame which has full unwind info.
