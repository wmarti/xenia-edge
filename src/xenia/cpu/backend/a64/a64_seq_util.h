...
inline void FixupVmxNan_V128_Fma(A64Emitter& e, int a, int c, int b, int tmp,
  ...
  auto& slow = e.AddToTail(
      [a, c, b, tmp, qs, &done](A64Emitter& e, Xbyak_aarch64::Label&) {
        // Lowest priority first, so an earlier operand overwrites a later one:
        // C, then B, then A. BIF inserts an operand wherever its self-compare
        // is false, which is exactly where that operand is NaN.
        ...
        // Quiet whatever NaN each lane ended up with. A lane still holding the
        // arithmetic result is either not NaN or already the default NaN, so
        // this only ever quiets an operand that was signalling.
