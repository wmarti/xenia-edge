...
struct DELAY_EXECUTION
  ...
  static void Emit(X64Emitter& e, const EmitArgType& i) {
    if (i.instr->flags & hir::DELAY_EXECUTION_INJECTED) {
      // SpinWaitInjectionPass tags. The counter/escalation machinery is
      // a64-only; on x64 the tag degrades to a plain pause rather than
      // silently inheriting the db16cyc hint semantics.
      e.pause();
      return;
    }
