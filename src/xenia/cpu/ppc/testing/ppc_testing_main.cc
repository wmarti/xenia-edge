...
DEFINE_int32(jit_corpus_top, 20,
             ...
             "CPU");
DEFINE_bool(jit_corpus_allow_incomplete, false,
...
struct WideMoveStats {
  ...
  // because it is one per materialization site in the emitted code. This is what
  // lets `stable` charge a fixed price per materialization instead of excluding
  // the whole thing -- see the stable computation for why excluding it made the
  // metric score the obvious optimization backwards.
...
// of one corpus and 3.5% between a capture and a replay, scaling with the number
// of call sites in a function (up to 7% on the largest).
//
...
// *host address* materialization -- which a single load from the backend context
// could replace -- versus guest immediates, which cannot be.
//
...
bool RunCorpusReplay() {
  ...
  for (size_t i = 0; i < page_addresses.size();) {
    ...
    const uint32_t size =
        static_cast<uint32_t>((j - i) * JitCorpus::kPageSize);
  ...
  // replacing a 3-instruction MOVZ/MOVK address chain with a single ldr from the
  // backend context moves that fragment from 0 counted instructions to 1, so
  // `stable` RISES by one while the emitted code shrinks by two. That is
  // precisely the change the "replaceable by one ldr" note below is inviting, so
  // the metric would have punished the fix it was pointing at.
  //
  // Charging one per chain is ASLR-invariant for the reason the classification
  // is: chain LENGTH depends on which 16-bit lanes of the load address happen to
  // be zero, but the NUMBER of chains is one per materialization site in the
  ...
  // whole difference was in the "32-bit-or-smaller" bucket. The reason is that a
  // 64-bit-form chain whose reconstructed value happens to fall below 4 GiB is
  // indistinguishable from a guest immediate, and Xenia does place host
