...
struct ScopedMultiWaiter {
  ...
  // previously fell back to state_mutex_ plus a condition variable, so a
  // suspend signal delivered while the thread held state_mutex_ (set_priority,
  // set_name, and every other WaitStarted path do) deadlocked that thread
  // against itself, and Resume could not recover it.
  //
   ...
   public:
    ...
    static std::pair<WaitResult, size_t> WaitMultiple(
      ...
      while (true) {
        ...
        // Park on the shared condvar (any signal wakes us); 1ms caps a lost
        // poke.
  ...
  class PosixCondition<Thread> final : public PosixConditionBase {
   ...
   public:
    ...
    bool Initialize(Thread::CreationParameters params,
      ...
      // size is too large. Retry with progressively smaller stacks before
      // giving up.
    ...
    ~PosixCondition() override {
      ...
      //  The proper way to handle them according to the webs is properly
      //  shutdown instead on relying on killing them using pthread_cancel.
      /*
...
#if XE_PLATFORM_ANDROID
        // pthread_getname_np was added in API 26 - below that, store the name
        // in this object, which may be only modified through Xenia threading,
        // but should be enough in most cases.
...
#else
        if (pthread_getname_np(thread_, result.data(), result.size() - 1) !=
            0) {
          assert_always();
        }
#endif
    ...
    int priority() const {
      ...
      // values 1..32 regardless of Darwin's SCHED_FIFO range
      // (typically 15..47).
      static const int fifo_min = sched_get_priority_min(SCHED_FIFO);
      static const int fifo_max = sched_get_priority_max(SCHED_FIFO);
   ...
   private:
    ...
    void post_execution_unlocked() override {
      ...
      // waiter can reach here before the thread has left its start routine.
      // That thread takes mutex_ in its own tail to publish the exit code, so
      // joining it while holding mutex_ deadlocked the pair permanently. Join
      // here, with the lock released, and only once.
      bool expected = false;
  ...
  class PosixTimer final : public PosixConditionHandle<Timer> {
   ...
   public:
    explicit PosixTimer(bool manual_reset)
        : PosixConditionHandle(manual_reset) {}
    ...
    bool SetRepeatingAfter(
        xe::chrono::hundrednanoseconds rel_time,
        std::chrono::milliseconds period,
        std::function<void()> opt_callback = nullptr) override {
  ...
  void* PosixCondition<Thread>::ThreadStartRoutine(void* parameter) {
...
#endif
    {
      ...
      // Resume() can land between, see count==0, bail, and wedge us at the
      // gate.
      if (create_suspended) {
  ...
  Thread* Thread::GetCurrentThread() {
    ...
    // TODO(bwrsandman): Disabling deleting thread_local current thread to
    // prevent
