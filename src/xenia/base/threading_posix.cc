...
class SuspendGate {
 public:
  ...
  // Must be called from the suspending thread before the signal is raised, not
  // from the handler. On the pipe implementation this is where the descriptors
  // are allocated: threads are never suspended in the common case, and two
  // descriptors per thread would be a real cost against macOS's low default
  // descriptor limit. Returns false if the gate cannot be armed, in which case
...
class PosixCondition<Thread> final : public PosixConditionBase {
 ...
 public:
  ...
  bool Initialize(Thread::CreationParameters params,
    ...
    // The start routine parks on the gate before running anything, so the gate
    // has to be armed here, by the creating thread, which can still report a
    // failure. Arming from the start routine would leave the caller believing
    // the thread is suspended when it is not.
